...
let channel = client.channels.get('470573951777767444')
if(member.user.bot) {
channel.send(`${member} واو جانا بوت شباب يلا نستمتع`)
}
})
...
client.on('message', message => {
  ...
  message.channel.createInvite({
    ...
    const embed = new Discord.RichEmbed()
        .setColor("RANDOM")
        .setDescription(" ✅   :heart:البوت ما يرفضلك طلب يا قلبي عطاك ياه خاص ")
...
client.on('message', message => {
            if(!message.channel.guild) return;
let args = message.content.split(' ').slice(1).join(' ');
if (message.content.startsWith('!bco')){
